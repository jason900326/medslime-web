...
| `player_entitlements` | Daily AI-detail service limit | Daily free detailed-explanation usage only; paid AI credit balances are retired |
| `payment_orders` | Payment ledger | Historical ECPay order ledger; new Pro / exam-detail fulfillment remains disabled until the new entitlement schema is deployed |
...
The planned paid content model is:
...
MedSlime Pro 30-day access (NT$149)
    -> advanced learning analysis / review tools
    -> AI follow-up questions: 10 per day
    -> does NOT automatically include every exam's paid full explanations
```

The paid entitlement tables and fulfillment RPC for these two products must be deployed before checkout is re-enabled.
